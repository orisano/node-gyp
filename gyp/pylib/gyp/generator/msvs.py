...
def _AddCustomBuildToolForMSVS(p, spec, primary_input,
  ...
  for config_name, c_data in py3compat.iteritems(spec['configurations']):
...
def _GenerateMSVSProject(project, options, version, generator_flags):
  ...
  for config_name, config in py3compat.iteritems(spec['configurations']):
...
def _ConvertToolsToExpectedForm(tools):
  ...
  """
  tool_list = []
  for tool, settings in py3compat.iteritems(tools):
    # Collapse settings with lists.
    settings_fixed = {}
    for setting, value in py3compat.iteritems(settings):
      if type(value) == list:
...
def _GetPrecompileRelatedFiles(spec):
  ...
  precompiled_related = []
  for _, config in py3compat.iteritems(spec['configurations']):
...
def _ExcludeFilesFromBeingBuilt(p, spec, excluded_sources, excluded_idl,
  ...
  for file_name, excluded_configs in py3compat.iteritems(exclusions):
...
def _DictsToFolders(base_path, bucket, flat):
  ...
  children = []
  for folder, contents in py3compat.iteritems(bucket):
...
def _GetPlatformOverridesOfProject(spec):
  ...
  for config_name, c in py3compat.iteritems(spec['configurations']):
...
def PerformBuild(data, configurations, params):
  ...
  for build_file, build_file_dict in py3compat.iteritems(data):
...
def _GetMSBuildProjectConfigurations(configurations):
  ...
  for (name, settings) in sorted(py3compat.iteritems(configurations)):
...
def _GetMSBuildGlobalProperties(spec, guid, gyp_file_name):
  ...
  for configuration in py3compat.itervalues(spec['configurations']):
...
def _GetMSBuildConfigurationDetails(spec, build_file):
  properties = {}
  for name, settings in py3compat.iteritems(spec['configurations']):
...
def _GetMSBuildPropertySheets(configurations):
  ...
  if not props_specified:
    ...
    sheets = []
    for condition, props in py3compat.iteritems(additional_props):
      import_group = [
        'ImportGroup',
        {'Label': 'PropertySheets',
         'Condition': condition
...
def _GetMSBuildConfigurationGlobalProperties(spec, configurations, build_file):
  ...
  properties = {}
  for (name, configuration) in sorted(py3compat.iteritems(configurations)):
    ...
    for name, value in sorted(py3compat.iteritems(tool_settings)):
...
def _GetMSBuildPropertyGroup(spec, label, properties):
  ...
  for name in reversed(properties_ordered):
    values = properties[name]
    for value, conditions in sorted(py3compat.iteritems(values)):
...
def _GetMSBuildToolSettingsSections(spec, configurations):
  ...
  for (name, configuration) in sorted(py3compat.iteritems(configurations)):
    ...
    for tool_name, tool_settings in sorted(py3compat.iteritems(msbuild_settings)):
  ...
  for source in sources:
    if isinstance(source, MSVSProject.Filter):
      ...
      if not source in sources_handled_by_action:
        ...
        for config_name, configuration in py3compat.iteritems(spec['configurations']):
...
def _GetMSBuildProjectReferences(project):
  ...
  if project.dependencies:
    ...
    for dependency in project.dependencies:
      ...
      for config in py3compat.itervalues(dependency.spec.get('configurations', {})):
...
def _GenerateMSBuildProject(project, options, version, generator_flags):
  ...
  for configuration in py3compat.itervalues(configurations):
...
def _GenerateActionsForMSBuild(spec, actions_to_add):
  ...
  actions_spec = []
  for primary_input, actions in py3compat.iteritems(actions_to_add):
    inputs = OrderedSet()
    outputs = OrderedSet()
    descriptions = []
    commands = []
