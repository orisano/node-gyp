...
def ConvertVCMacrosToMSBuild(s):
  ...
  if '$' in s:
    ...
    for old, new in py3compat.iteritems(replace_map):
      s = s.replace(old, new)
...
def ConvertToMSBuildSettings(msvs_settings, stderr=sys.stderr):
  ...
  """
  msbuild_settings = {}
  for msvs_tool_name, msvs_tool_settings in py3compat.iteritems(msvs_settings):
    if msvs_tool_name in _msvs_to_msbuild_converters:
      msvs_tool = _msvs_to_msbuild_converters[msvs_tool_name]
      for msvs_setting, msvs_value in py3compat.iteritems(msvs_tool_settings):
        if msvs_setting in msvs_tool:
          # Invoke the translation function.
          try:
            msvs_tool[msvs_setting](msvs_value, msbuild_settings)
          except ValueError as e:
            print('Warning: while converting %s/%s to MSBuild, %s' % (msvs_tool_name, msvs_setting, e), file=sys.stderr)
        else:
      ...
      print('Warning: unrecognized tool %s while converting to MSBuild.' % msvs_tool_name, file=sys.stderr)
...
def _ValidateSettings(validators, settings, stderr):
  ...
  for tool_name in settings:
    if tool_name in validators:
      ...
      for setting, value in py3compat.iteritems(settings[tool_name]):
        if setting in tool_validators:
          try:
            tool_validators[setting](value)
          except ValueError as e:
            print('Warning: for %s/%s, %s' % (tool_name, setting, e), file=sys.stderr)
        else:
      ...
      print('Warning: unrecognized tool %s' % tool_name, file=sys.stderr)
